...
echo '<table border="1" id="call-table">';
echo '<thead>';
echo '<tr>';
...
echo '<th style="min-width:64px;" title="Inclusive Time">In WT &#956;s</th>';
echo '<th style="min-width:64px;" title="Exclusive Time">Ex &#956;s</th>';
// echo '<th style="min-width:64px;" title="Inclusive Time">In CPU &#956;s</th>';
...
foreach ($tree as $func=>$perf) {
    ...
    // Inclusive Wall Time
    echo '<td class="r">';
    echo _show_stat($perf['xdebug']['iwt'], $perf['xhprof']['iwt']);
    ...
    echo '<td class="r">XMU' . ($perf['xhprof']['xmu']) . '</td>';
    // echo '<td class="r">' . ($perf['xhprof']['imp'] - $perf['xhprof']['xmp']) . '</td>'; // <td class="r">&mdash;</td>'; // Exclusive Memory Peak
    echo '<td class="r">XMP' . ($perf['xhprof']['xmp']) . '</td>';
    echo '<td class="r">XCPU' . ($perf['xhprof']['xcpu']) . '</td>';

    // Inclusive WT
    echo '<td class="r">IWT' . _show_stat($perf['xdebug']['iwt'], ($perf['xhprof']['xwt'] - $perf['xhprof']['iwt']) ) . '</td>';
